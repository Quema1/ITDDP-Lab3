...
public class SecurityHandler implements SOAPHandler<SOAPMessageContext> {
	...
	public SecurityHandler() throws JAXBException {
		jaxb = JAXBContext.newInstance("org.example.service");
	}
...
//		return true;
		try {
			...
			var isValid = isValidToken(sh.getValue());
			if (isValid) return true;
			else {
				throw new SOAPFaultException(createFault(message, "Client token invalid"));
			}
		} catch (Exception e) {
	...
	private boolean isValidToken(String token) {
		if (token == null || token.isEmpty() || !token.equals(CLIENT_TOKEN)) {
